...
class SlackApi:
    ...
    def get_channels_by_names(self, channels_names: str):
        return {k: v['name'] for k, v in self.get_channels()
                if v['name'] in channels_names}

    def get_channels_by_ids(self, channels_ids: str):
        return {k: v['name'] for k, v in self.get_channels()
                if k in channels_ids}

    def get_users_by_names(self, user_names: str):
        return {k: v['name'] for k, v in self.get_users()
                if v['name'] in user_names}

    def get_users_by_ids(self, users_ids: str):
        return {k: v['name'] for k, v in self.get_users()
                if k in users_ids}
    ...
    @classmethod
    def create_using_queries(cls, integration_name: str,
                             init_usergroups: bool = False):
    ...
    @classmethod
    def create_from_dict(cls, slack_workspace: Mapping[str, Any],
